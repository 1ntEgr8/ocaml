...
let simplify_exits lam =
  ...
  | Lmarker(_m, l) -> count ~try_depth l
  ...
  *)
  ...
  | Lmarker(m, l) -> Lmarker (m, simplif ~try_depth l)
  in
...
(* This (small)  optimisation is always legal, it may uncover some
  ...
  | Lmarker(m, l) -> Lmarker(m, simplif l)
  in
  simplif lam
